...
/* print_banner - ASCII art title at program startup (console). */
static void print_banner(void)
{
    text_table_print_project_ascii_art(stdout);
}
...
/*
 * read_int_in_range - Read one integer from stdin; retry until value is in
 * [minVal, maxVal].
 */
static void read_int_in_range(const char *prompt, int minVal, int maxVal,
                              int *out) {
  int value;
  int readCount;
...
/*
 * read_display_floor - Read a building floor number (ground=0,
 * negative=basement).
 */
static int read_display_floor(const Simulation *sim, const char *prompt,
                              int *outDisplay) {
  int display;
  int minDisplay;
  int maxDisplay;
...
static void configure_interactively(SimulationConfig *config) {
  ...
  snprintf(prompt, sizeof(prompt),
           "Floors above ground (%d-%d): ", MIN_FLOORS_ABOVE_GROUND,
           MAX_FLOORS_ABOVE_GROUND);
  read_int_in_range(prompt, MIN_FLOORS_ABOVE_GROUND, MAX_FLOORS_ABOVE_GROUND,
                    &floorsAboveGround);

  snprintf(prompt, sizeof(prompt),
           "Underground floors below ground (%d-%d): ", MIN_UNDERGROUND_FLOORS,
  ...
  for (;;) {
    ...
    printf("Building too large (max %d internal levels). Adjust floors and try "
           "again.\n",
           MAX_TOTAL_FLOORS);
    snprintf(prompt, sizeof(prompt),
             "Floors above ground (%d-%d): ", MIN_FLOORS_ABOVE_GROUND,
  ...
  printf("Building: %d above ground + ground -> display floors %d .. %d",
         floorsAboveGround, config_display_floor_min(config),
  ...
  snprintf(prompt, sizeof(prompt),
           "Number of elevators (%d-%d): ", MIN_ELEVATORS, MAX_ELEVATORS);
  read_int_in_range(prompt, MIN_ELEVATORS, MAX_ELEVATORS, &elevators);

  snprintf(prompt, sizeof(prompt), "Elevator capacity (%d-%d): ", MIN_CAPACITY,
           MAX_CAPACITY);
...
/*
 * add_passenger_interactive - Menu option 2: one request if sim already
 * initialized. Requires prior option 1; does not run simulation_run by itself.
 */
...
/*
 * start_simulation_interactive - Menu option 1: configure and initialize a
 * fresh manual simulation. Does not auto-run; user can add requests and run via
 * option 3.
 */
static void start_simulation_interactive(Simulation *sim,
  ...
  printf("Simulation initialized. Add manual requests (option 2), then run "
         "(option 3).\n");
}

/*
 * generate_random_seed_interactive - Menu option 5: user picks building params
 * and request count; generates random trips and saves to random_seed.txt.
 */
static void generate_random_seed_interactive(Simulation *sim,
                                             SimulationConfig *config) {
...

  {
    char prompt[96];
    snprintf(prompt, sizeof(prompt),
             "How many random passenger requests (%d-%d): ", MIN_SEED_REQUESTS,
             MAX_SEED_REQUESTS);
    read_int_in_range(prompt, MIN_SEED_REQUESTS, MAX_SEED_REQUESTS,
                      &numRequests);
  }
  ...
  /* randomSeed 0 => seed_generate_random uses clock; value is stored in the
   * seed file */
  ...
  printf("Saved %d random requests to %s (clock seed %u — use option 6 to "
         "replay)\n",
...
/*
 * load_seed_and_run_interactive - Menu option 6: load random_seed.txt and run
 * DES.
 */
static void load_seed_and_run_interactive(Simulation *sim,
                                          SimulationConfig *config) {
  SeedScenario scenario;
...
static void run_current_simulation(Simulation *sim) {
  if (sim == NULL || sim->numFloors == 0) {
    printf("Simulation not initialized. Start a new simulation first (option "
           "1).\n");
    return;
  }
  simulation_print_state(sim, 0);
 ...
 * (lowest T in Future Event List). No sleep() between events. See
 * docs/DES_COURSE_HE.md.
 */
int main(void) {
  Simulation sim;
  SimulationConfig config;
